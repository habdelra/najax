...
function najax (uri, options, callback) {
  ...
  var req = (ssl ? https : http).request(options, function requestHandler (res) {
    ...
    res.on('data', function onData (d) { data += d })
  ...
  function onError (e) {
    ...
    if (_.isFunction(o.error)) o.error(jqXHR, jqXHR.statusText, e)
    // jqXHR, statusText, error
